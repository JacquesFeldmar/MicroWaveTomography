...
Trois programmes, plus une bibliothèque. Rien d'autre : l'accès à l'instrument
et la calibration SOL viennent de `../commun/`, partagé avec le projet de tomographie.
...
| *`noyau.py`* | *bibliothèque du projet : modèle stratifié, inversion, simulateur* | *jamais lancée* |
...
## 9. Régénérer ce guide en PDF

`docs/GUIDE_UTILISATION.pdf` se fabrique à partir de ce fichier :

```
python construire_guide.py
```

pandoc écrit une page HTML **autonome** — la feuille de style `guide_style.css` et les
copies d'écran de `docs/ecrans/` y sont intégrées —, puis Chrome l'imprime en PDF. Il
faut donc pandoc, Google Chrome et les quatre copies d'écran citées plus haut.

Le programme **contrôle le PDF avant de remplacer l'ancien** : il doit employer une
police de la feuille de style et contenir chaque image citée. Sinon il s'arrête avec le
code de retour 1, et le PDF livré reste en place.

> Ce contrôle existe parce que le défaut s'est produit. Du 21 août au 10 septembre 2026,
> le PDF livré était en Times New Roman, avec des icônes d'image cassée à la place des
> copies d'écran : la page intermédiaire avait été écrite hors de ce dossier, ses liens
> relatifs ne menaient plus nulle part, et Chrome avait imprimé sans rien dire.

Pour juger un PDF existant sans rien régénérer :

```
python construire_guide.py --controle docs/GUIDE_UTILISATION.pdf
```
...
```
noyau.py            bibliothèque du projet : modèle, inversion, simulateur
../commun/          partagé avec la tomographie : instrument, SOL, traitement
...
theorie/            sources LaTeX du document de théorie, figures_*.py
construire_guide.py → docs/GUIDE_UTILISATION.pdf  (guide_style.css, docs/ecrans/)
```

Les `figures_*.py` ne servent qu'à produire les figures du document de théorie. Ils
sont rangés avec les sources LaTeX dans `theorie/`, d'où il faut compiler.
