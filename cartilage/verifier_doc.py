...
COMMUN = os.path.join(os.path.dirname(ICI), "commun")
PYTHON = sys.executable

PROGRAMMES = ["calibration.py", "radar.py", "qualification.py"]
BIBLIOTHEQUES = ["noyau.py"]
GUIDE = "GUIDE_UTILISATION.md"
LATEX = [os.path.join("theorie", f) for f in
         ("cours_theorie.tex", "annexe_phase1.tex", "guide_phase1.tex")]
...
def verifie_fichiers(guide):
    ...
    for f in sorted(set(re.findall(r"`?([a-z_]+\.py)`?", guide))):
        if os.path.exists(os.path.join(ICI, f)):
            dire(f"   {f:26s} racine")
        elif os.path.exists(os.path.join(COMMUN, f)):
            dire(f"   {f:26s} commun/")
...
def verifie_latex():
    ...
    for doc in LATEX:
        ...
        for f in sorted(cites):
            if (os.path.exists(os.path.join(ICI, f))
                    or os.path.exists(os.path.join(COMMUN, f))):
